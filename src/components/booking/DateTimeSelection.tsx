...
const DateTimeSelection = ({ bookingData, setBookingData }: DateTimeSelectionProps) => {
  ...
  const fetchBookedTimes = async () => {
    ...
    try {
      ...
      const appointments = await appointmentStore.getAppointmentsByBarber(
      ...
      const fullDayUnavailable = unavailableSlots.some(slot => slot.isWholeDay);
      setIsFullDayUnavailable(fullDayUnavailable);
      
      const unavailableTimesForDate = unavailableSlots
        .filter(slot => !slot.isWholeDay && slot.time)
        .map(slot => slot.time!);
    ...
    <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        {!selectedDate ? (
          ...
          <p className="text-sm sm:text-base text-muted-foreground text-center py-6 sm:py-8">This barber is unavailable on this date</p>
        ) : (
